...
| 📁 | [**云盘助手**](./extensions/cloud-drive-helper/) | 115、光鸭、123 | 统一配置页；115 分客户端扫码、光鸭双方式连接、123 账号密码登录；目录只读 | `preview` |
...
### 云盘助手：统一配置与目录选择（0.4.0）

1. 加载或重新加载 `extensions/cloud-drive-helper/`，点击扩展图标打开完整配置页。页面提供 115、光鸭、123 三个入口，支持 115 手机扫码、光鸭双方式连接和 123 账号密码登录。
...
新增唯一主机权限 `https://api.123278.com/*`，请求独立限流至少 2 秒。接口与签名参考 [123 官网](https://yun.123pan.cn/)当前网页客户端及 [OpenList 实现](https://github.com/OpenListTeam/OpenList/blob/main/drivers/123/util.go)，使用网站接口而非开发者 AppID。若出现人机验证，停止并提示；暂不支持验证码、自动续期、分享转存或磁力离线。26 项合成测试及 Chrome 合成页面的密码清空、分页、目标保存和断开验证通过；**真实账号密码登录与目录读取尚未验收**，网站接口或风控可能需要后续适配。

后续升级：115 暂停在根目录第一层文件夹读取与目标保存。分享转存、磁力离线及各自的结果确认留待后续单独接入；不预先增加安全密钥流程，只有具体接口实测明确要求时再处理。123 本阶段接入账号密码登录、第一层文件夹与目标保存，真实账号链路待验收。
